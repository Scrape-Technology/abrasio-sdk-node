...
import { AbrasioError, AuthenticationError, SessionError, InsufficientFundsError, RateLimitError, TimeoutError, } from '../exceptions.js';
const MAX_RETRIES = 3;
...
export class AbrasioAPIClient {
    ...
    async createSession(options = {}) {
        ...
        if (options.mobileModel)
            payload.mobile_model = options.mobileModel;
        if (options.proxy) {
    ...
    async waitForReady(sessionId, timeoutSeconds = 60, pollInterval = 1.0) {
        let elapsed = 0;
        ...
        throw new TimeoutError(`Session ${sessionId} did not become ready within ${timeoutSeconds}s`, timeoutSeconds * 1000);
    }
