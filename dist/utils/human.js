...
function generateControlPoints(start, end, numControl = 2) {
    const points = [start];
...
function calculateMovementTime(distance, minTime = 0.1, maxTime = 1.5) {
    if (distance < 1)
        return minTime;
...
function addJitter(point, amount) {
...
// ── Public API ──
export async function humanMoveTo(page, x, y, options = {}) {
...
export async function humanClick(page, options = {}) {
...
export async function humanType(page, text, options = {}) {
    const { minDelayMs = 30, maxDelayMs = 150, mistakeProbability = 0.02, thinkPauseProbability = 0.05, } = options;
    if (options.selector) {
...
export async function humanWait(minSeconds = 0.5, maxSeconds = 2.0) {
...
`;
const _humanizedPages = new WeakSet();
/**
 ...
 */
export async function humanizePage(page, opts = {}) {
    if (_humanizedPages.has(page))
        return;
    ...
    page.hover = async (selector, options) => {
        ...
        return origHover(selector, options);
    };
}
/**
 ...
 */
export async function humanizeContext(ctx, opts = {}) {
    const { headless = false, speedFactor = 1.0 } = opts;
    for (const page of ctx.pages()) {
        try {
            await humanizePage(page, { headless, speedFactor });
        }
        catch { /* ignore */ }
    }
    ctx.on('page', (page) => {
        humanizePage(page, { headless, speedFactor }).catch(() => { });
    });
}
