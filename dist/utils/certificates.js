 ...
 */
export function buildClientCertificate(origin, options = {}) {
    if (!origin) {
    ...
    if (!hasPem && !hasPfx) {
        throw new AbrasioError('buildClientCertificate requires either both cert/certPath and key/keyPath (PEM), ' +
    ...
    if (pfx !== undefined)
        entry.pfx = pfx;
    if (pfxPath !== undefined)
        entry.pfxPath = pfxPath;
 ...
 */
function resolveCertMaterial(certificate) {
    if (certificate.pfx || certificate.pfxPath) {
        const pfxBytes = certificate.pfx ?? fs.readFileSync(certificate.pfxPath);
    ...
    if (!cert || !key) {
        throw new AbrasioError('resolveCertMaterial requires either certPath/cert + keyPath/key (PEM), or pfxPath/pfx (PFX/PKCS12).');
    }
...
function buildAgent(proxyUrl) {
    const baseOptions = { rejectUnauthorized: true };
    return proxyUrl
        ? new HttpsProxyAgent(proxyUrl, baseOptions)
        : new https.Agent(baseOptions);
}
function performRequest(method, targetUrl, headers, body, agent, certMaterial, timeoutMs) {
 ...
 */
export async function routeWithClientCertificate(target, url, certificate, options = {}) {
    ...
    await target.route(url, async (route, request) => {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                const resp = await performRequest(request.method(), request.url(), headers, request.postDataBuffer(), agent, certMaterial, timeoutMs);
                await route.fulfill({
                ...
                if (attempt < retries) {
                    ...
                    console.warn(`[abrasio] Certificate route replay attempt ${attempt + 1}/${retries + 1} failed for ` +
