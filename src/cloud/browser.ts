...
export class CloudBrowser {
    ...
    private _apiClient: AbrasioAPIClient | null = null;
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            this._sessionId = sessionData['id'] as string;
            if (!this._sessionId) {
            ...
            this._wsEndpoint = session['ws_endpoint'] as string;
            if (!this._wsEndpoint) {
                throw new SessionError('No WebSocket endpoint returned', this._sessionId);
            }

            // Live view URL
            const liveViewUrl = session['live_view_url'] as string | undefined;
            if (liveViewUrl) {
